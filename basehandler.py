...
class BaseHandler(webapp.RequestHandler):
    ...
    def init_facebook(self):
        ...
        if facebook.user_id:
            ...
            user = retrieveCache(facebook.user_id, User)
            if user:
            ...
            if not user and facebook.access_token:
                ...
                try:
                    ...
                    user = User(key_name=facebook.user_id,
                        ...
                        email=me.get(u'email'), picture=me[u'picture'][u'data'][u'url'], last_hosted=datetime.datetime.min, display_type=2, achievements=[], first_time=True, current_game=None, last_active=datetime.datetime.now())
                    #user.put()
                    storeCache(user, user.user_id)
                except Exception, ex:
                    logging.debug(ex)
